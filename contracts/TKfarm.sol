...
contract TKFarm is Ownable {
    ...
    uint256 public sushiPerBlock = 10 ;
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.sushi.mint(devaddr, sushiReward.div(10));
        pool.sushi.mint(address(this), sushiReward);
    ...
    function safeSushiTransfer(address _to, SushiToken _sushi, uint256 _amount) internal {
        uint256 sushiBal = _sushi.balanceOf(address(this));
        if (_amount > sushiBal) {
            _sushi.transfer(_to, sushiBal);
        } else {
            _sushi.transfer(_to, _amount);
        }
    }
